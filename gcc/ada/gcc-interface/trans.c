      ...
      Int gigi_operating_mode)
{
  ...
  if (Stack_Check_Probes_On_Target)
    ...
    set_stack_check_symbol ("__gnat_stack_limit");
  else
  ...
  /* Otherwise we have an iteration scheme and the condition is given by the
      ...
      else
	{
	 ...
	 which works in all cases.  */
      ...
      if (optimize_cfg && !optimize_debug)
	{
  ...
  if (at_end)
    {
      ...
      /* When not optimizing CFG, disable inlining of finalizers as this can
...
gnat_to_gnu (Node_Id gnat_node)
{
  ...
  switch (kind)
    ...
    case N_Null_Statement:
      /* When not optimizing CFG, turn null statements from source into gotos
       * to the next statement that the middle-end knows how to preserve.  */
      if (!optimize_cfg && Comes_From_Source (gnat_node))
	{
	...
	if (Present (Expression (gnat_node)))
	  {
		...
		/* And find out whether this is a candidate for Named Return
		   ...
		   optimization when we're preserving the control flow as it
		   entails hoisting the allocation done below.  */
		if (optimize_cfg
		    && !optimize_debug
	...
	if (gnu_return_label_stack->last ())
	  {
	    ...
	    /* When not optimizing CFG, make sure the return is preserved.  */
	    if (!optimize_cfg && Comes_From_Source (gnat_node))
  ...
  /* When not optimizing CFG, turn boolean rvalues B into B != false tests
