...
--  This is the Cert specific version of s-gcmain.adb.
...
package body System.Generic_C_Math_Interface is
   ...
   --  the arguments or the C function result. Prescribed results are
   --  satisfied by referring to corresponding requirements in C,
   --  standard implementation practice or by explicit special-casing
   --  in the code below.

   --  If one of the arguments of a function is a NaN, the function
   --  will return a NaN value or raise Argument_Error. Generally, for
   --  functions that require Argument_Error to be raised for some
   --  arguments will also raise Argument_Error for NaN arguments.

   --  Many comparisons for special cases are inverted using "not" in
   --  order to make sure the condition is false for NaN values,
   --  using the principle that any comparison involving a NaN argument
   --  evaluates to false.
