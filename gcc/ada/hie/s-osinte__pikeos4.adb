...
--  This is the Ravenscar version of this package for PikeOS 4.x
...
package body System.OS_Interface is
   ...
   procedure Init_Thread (Id : Thread_Id);
   --  Set components of Id.
   ...
   function p4_my_uid_syscall return P4_uid_t;
   pragma Import (C, p4_my_uid_syscall);

   function p4_thread_stop_syscall
     (Tnum : P4_thr_t; Flags : P4_uint32_t) return P4_e_t;
   pragma Import (C, p4_thread_stop_syscall);

   function p4_thread_resume (Tnum : P4_thr_t) return P4_e_t;
   pragma Import (C, p4_thread_resume);

   function p4_thread_ex_sched_syscall
     (Tnum     : P4_thr_t;
      Old_Prio : Address;
      Old_Tp   : Address;
      New_Prio : P4_prio_t;
      ...
      Flags    : P4_uint32_t)
     return P4_e_t;
   pragma Import (C, p4_thread_ex_sched_syscall);

   P4_THREAD_SCHED_FIRST : constant := 2**0;
   ...
   procedure p4_tls_init (tls : Address);
   pragma Import (C, p4_tls_init, "__gnat_p4_tls_init");

   procedure p4_tls_register (tls : Address);
   pragma Import (C, p4_tls_register);

   Max_Thread_Num : constant P4_thr_t := 512;
   --  Maximum number of threads that can be created

   type Thread_Id_Array is array (P4_thr_t range 0 .. Max_Thread_Num - 1) of
     Thread_Id;
   ...
   --  Number of the next thread to be created
   ...
   function p4_thread_get_priority (Tnum : P4_thr_t; Prio : Address)
                                   return P4_e_t is
   begin
      return p4_thread_ex_sched_syscall (Tnum, Prio, Null_Address,
   ...
   function p4_my_thread return P4_thr_t is
      Uid : constant P4_uid_t := p4_my_uid_syscall;
   ...
   procedure Init_Thread (Id : Thread_Id) is
   begin
      All_Threads (Next_Thread) := Id;
      Id.Num := Next_Thread;

      Id.Interrupt := No_Interrupt;
   end Init_Thread;
   ...
   begin
      ...
      Init_Thread (Environment_Thread);

      --  Initialize TLS

      p4_tls_init (Environment_Thread.Tls'Address);
      p4_tls_register (Environment_Thread.Tls'Address);
   ...
   procedure Set_Interrupt (Id : Interrupt_ID) is
      Thread : constant Thread_Id := Thread_Self;
   begin
      --  Can only be set once

      pragma Assert (Thread.Interrupt = No_Interrupt);
   ...
   is
      ...
      function Gnat_p4_thread_create
        (Num        : P4_thr_t;
         ...
         Stack      : Address;
         Stack_Size : System.Parameters.Size_Type;
         Tls_Addr   : Address) return P4_e_t;
   ...
   begin
      ...
      Status := Gnat_p4_thread_create
        (Id.Num, P4_prio_t (Priority), Code, Arg,
         Stack_Address, Stack_Size, Id.Tls'Address);
   ...
   procedure Sleep is
      Status : P4_e_t;
   begin
      Status := p4_thread_stop_syscall (p4_my_thread, 0);
      pragma Assert (Status = P4_E_OK);
   end Sleep;
