...
package System.OS_Interface is
   ...
   P4_E_PAGEFAULT  : constant P4_e_t := 10;
   P4_E_INVAL      : constant P4_e_t := 13;
   P4_E_BADTIMEOUT : constant P4_e_t := 16;
   P4_E_BADUID     : constant P4_e_t := 17;
   P4_E_TIMEOUT    : constant P4_e_t := 25;
   P4_E_LIMIT      : constant P4_e_t := 26;
   P4_E_STATE      : constant P4_e_t := 28;
   P4_E_CANCEL     : constant P4_e_t := 29;
   P4_E_ABORT      : constant P4_e_t := 30;
   ...
   type P4_mutex_t is limited private;

   P4_MUTEX_RECURSIVE     : constant := 2 ** 0;
   P4_MUTEX_ROBUST        : constant := 2 ** 1;
   P4_MUTEX_CANCELABLE    : constant := 2 ** 2;
   P4_MUTEX_PRIO          : constant := 2 ** 28;
   P4_MUTEX_SHARED        : constant := 2 ** 31;
   P4_MUTEX_MAX_RECURSION : constant P4_uint32_t := P4_uint32_t'Last;

   procedure p4_mutex_init
     (mutex : access P4_mutex_t;
      flags : P4_uint32_t)
     with Import, External_Name => "p4_mutex_init", Convention => C;

   function p4_mutex_lock
     (mutex : access P4_mutex_t;
      timeout : P4_timeout_t) return P4_e_t
     with Import, External_Name => "p4_mutex_lock", Convention => C;

   function p4_mutex_unlock (mutex : access P4_mutex_t) return P4_e_t
     with Import, External_Name => "p4_mutex_unlock", Convention => C;

   --  Conditional variable definitions

   type P4_cond_t is limited private;

   P4_COND_WAKE_ALL : constant := 2 ** 0;
   P4_COND_PRIO     : constant := 2 ** 28;
   P4_COND_SHARED   : constant := 2 ** 31;

   procedure p4_cond_init
     (cond  : access P4_cond_t;
      flags : P4_uint32_t)
     with Import, External_Name => "p4_cond_init", Convention => C;

   function p4_cond_wait
     (cond    : access P4_cond_t;
      mutex   : access P4_mutex_t;
      timeout : P4_timeout_t) return P4_e_t
     with Import, External_Name => "p4_cond_wait", Convention => C;

   function p4_cond_wake
     (cond  : access P4_cond_t;
      flags : P4_uint32_t) return P4_e_t
     with Import, External_Name => "p4_cond_wake", Convention => C;
...
private
   ...
   type P4_rulock_t is record
      Lock     : Interfaces.C.unsigned := 0;
      Wait_Uid : P4_uid_t := 0;
      List     : P4_rulock_list_elem_t := (others => System.Null_Address);
   end record with Convention => C;

   type P4_mutex_t is record
      Lock  : P4_rulock_t;
      Count : P4_uint32_t := 0;
      Flags : P4_uint32_t := 0;
   end record with Convention => C;

   type P4_ulock_t is record
      Lock     : Interfaces.C.unsigned := 0;
      Wait_Uid : P4_uid_t := 0;
   end record;

   type P4_cond_t is record
      Lock : P4_ulock_t;
   end record with Convention => C;
