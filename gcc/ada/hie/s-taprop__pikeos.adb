...
--  This is the pikeos version of this package
...
package body System.Task_Primitives.Operations is
   ...
   begin
      ...
      Result :=
        p4_mutex_lock
          (mutex   => Self_ID.Common.LL.Lock'Access,
           timeout => P4_TIMEOUT_INFINITE);
      pragma Assert (Result = P4_E_OK);

      --  If Wakeup was already called before Self_ID.Common.LL.Lock was
      --  locked, we simply keep running (don't call pthread_cond_wait)

      if not Self_ID.Common.LL.Wakeup_Signaled then
         Result :=
           p4_cond_wait
             (cond    => Self_ID.Common.LL.Sleep_CV'Access,
              mutex   => Self_ID.Common.LL.Lock'Access,
              timeout => P4_TIMEOUT_INFINITE);
      ...
      Self_ID.Common.LL.Wakeup_Signaled := False;

      Result := p4_mutex_unlock (mutex => Self_ID.Common.LL.Lock'Access);
      pragma Assert (Result = P4_E_OK);
   end Sleep;
   ...
   begin
      ...
      pragma Assert (not T.Common.LL.Wakeup_Signaled);
      T.Common.LL.Wakeup_Signaled := True;

      Result :=
        p4_cond_wake (cond  => T.Common.LL.Sleep_CV'Access, flags => 0);
      pragma Assert (Result = P4_E_OK);

      Result := p4_mutex_unlock (mutex => T.Common.LL.Lock'Access);
      pragma Assert (Result = P4_E_OK);
   end Wakeup;
