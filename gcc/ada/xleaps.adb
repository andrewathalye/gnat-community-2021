...
--  leap second occurrences. It uses several target dependent values taken
--  from a-calend.ads to perform its computations.

--  The algorithm behind this utility calculates the total number of days
--  up to a leap second occurrence from a target dependent origin of time.
...
--     1) Increment Leap_Seconds_Count in a-calend.adb.
--     2) Increment LS_Count in xleaps.adb.
--     3) Add the new date to the aggregate of array LS_Dates in xleaps.adb.
--     4) Compile and execute xleaps for common targets.
--     5) Replace the values of array Leap_Second_Times in a-calend.adb with
--        the aggregate generated by xleaps.
...
procedure XLeaps is
   ...
   --  second occurrence. The type is explicitly declared to be large enough
   --  and system independent.

   type Units is range -2 ** 63 .. +2 ** 63 - 1;

   --  Target specific units (currently all targets use nano-seconds)
...
begin
   ...
   for Index in 1 .. LS_Count loop
      ...
      --  leap second occurrence. Note that non-leap centenial years are not
      ...
      --  leap seconds that have occurred up to the current one (Index - 1).
      --  The seconds are then converted to target-dependent units.
