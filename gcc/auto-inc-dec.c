...
attempt_change (rtx new_addr, rtx inc_reg)
{
  ...
  /* Emit a debug insn right before the memory access in which the increment
     (or decrement) is folded to provide information about the incremented
     (or decremented) register.  */
  if (MAY_HAVE_DEBUG_INSNS && REG_EXPR (inc_insn.reg_res))
    {
      rtx bind = gen_rtx_VAR_LOCATION (GET_MODE (inc_insn.reg_res),
				       REG_EXPR (inc_insn.reg_res),
				       inc_insn.reg_res,
				       VAR_INIT_STATUS_INITIALIZED);
      rtx_insn *insn = emit_debug_insn_before (bind, mem_insn.insn);
      INSN_LOCATION (insn) = INSN_LOCATION (inc_insn.insn);
      df_insn_rescan (insn);
    }
