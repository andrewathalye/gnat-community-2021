		 ...
		 gimple_seq *post_p ATTRIBUTE_UNUSED)
{
  ...
  switch (code)
    {
    case BIND_EXPR:
      /* If we have a toplevel BIND_EXPR corresponding to a function
	 body and we're requested to dump SCOs, do so.  */
      if (flag_dump_scos
	  && BIND_EXPR_BLOCK (*expr_p)
	  && (TREE_CODE (BLOCK_SUPERCONTEXT (BIND_EXPR_BLOCK (*expr_p)))
	      == FUNCTION_DECL))
	generate_scos (*expr_p);
      break;
