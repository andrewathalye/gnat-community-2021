...
move_debug_insns_from_forwarder_block (basic_block bb)
{
  ...
  rtx_insn *after, *insn, *curr;

  /* Move debug insns only if the successor has a single predecessor.  */
  if (succ == EXIT_BLOCK_PTR_FOR_FN (cfun) || !single_pred_p (succ))
    return;
	  ...
	  if (FORWARDER_BLOCK_P (target)
	      ...
	      else if (!optimize_cfg)
		{
		  ...
		  if (LOCATION_LOCUS (new_locus) != UNKNOWN_LOCATION
			...
			{
			  /* Preserve the location of debug insns if the block
			     doesn't contain anything else and they cannot be
			     moved to the single successor.  */
			  if (DEBUG_INSN_P (BB_END (target))
			      && !single_pred_p (single_succ (target))
			      && !single_succ_p (single_succ (target)))
			    new_locus = INSN_LOCATION (BB_END (target));
			  else
  ...
  if (! targetm.cannot_modify_jumps_p ())
      ...
      do
	{
	  ...
	  for (b = ENTRY_BLOCK_PTR_FOR_FN (cfun)->next_bb; b
		...
		{
		  edge e = single_succ_edge (b);

		  /* If we are preserving the CFG and the edge holds an unique
		     locus, we cannot redirect the jump because this would
		     create a non-fallthru edge to the exit block that the
		     fixup code in fixup_reorder_chain cannot handle.  */
		  if (flag_preserve_control_flow
		      && LOCATION_LOCUS (e->goto_locus) != UNKNOWN_LOCATION
		      && e->goto_locus != INSN_LOCATION (BB_END (b)))
		    ;
		  else if (redirect_jump (as_a <rtx_jump_insn *> (BB_END (b)),
					  PATTERN (ret), 0))
		    {
		      if (use)
			emit_insn_before (copy_insn (PATTERN (use)),
		      ...
		      if (MAY_HAVE_DEBUG_INSNS
			  && ANY_RETURN_P (PATTERN (BB_END (b))))
			{
			  rtx_insn *insn;

			  FOR_BB_INSNS (single_succ (b), insn)
			    if (DEBUG_INSN_P (insn))
