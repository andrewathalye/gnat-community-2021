...
unique_locus_on_edge_between_p (basic_block a, basic_block b, bool skip_a)
{
  ...
  if (!useful_location_p (cfun, goto_locus))
    return false;
  ...
  if (optimize && single_succ_p (b))
    {
      ...
      /* If B is the end of function's extended block, A (most likely) ends
	 with a user RETURN.  If the function returns a value, there will be
	 ...
	 out later, so pretend that the RETURN holds a unique locus.  */
	  ...
	  && INSN_P (insn)
	  && GET_CODE (PATTERN (insn)) != USE
	  && LOCATION_LOCUS (INSN_LOCATION (insn)) == cfun->function_end_locus
	  && (c == EXIT_BLOCK_PTR_FOR_FN (cfun)
	      || (single_succ_p (c)
		  && single_succ (c) == EXIT_BLOCK_PTR_FOR_FN (cfun))))
	return true;

      if (c != EXIT_BLOCK_PTR_FOR_FN (cfun))
	{
	  insn = BB_HEAD (c);
...
rtl_merge_blocks (basic_block a, basic_block b)
{
  ...
  /* If not optimizing CFG, preserve the locus of the single edge between
...
struct edge_hasher : nofree_ptr_hash <edge_def>
{
  static inline hashval_t hash (edge);
  static bool equal (edge, edge);
};

inline hashval_t
edge_hasher::hash (edge e)
{
  return (hashval_t) (intptr_t) e->aux;
}

bool
edge_hasher::equal (edge e1, edge e2)
{
  rtx_insn *insn1, *insn2;

  for (insn1 = e1->insns.r, insn2 = e2->insns.r;
       insn1 && insn2;
       insn1 = next_nonnote_nondebug_insn (insn1),
       insn2 = next_nonnote_nondebug_insn (insn2))
    {
	...
	return false;
      ...
      if (INSN_LOCATION (insn1) != INSN_LOCATION (insn2))
	return false;
    }
...
hash_edge (edge e)
{
  ...
  for (insn = e->insns.r; insn; insn = next_nonnote_nondebug_insn (insn))
    if (INSN_P (insn) && GET_CODE (PATTERN (insn)) == SET)
      inchash::add_rtx (PATTERN (insn), hstate);
    else
      {
	/* Bail out with a stable hash value.  */
	hstate.add_int (e->src->index);
	break;
      }
...
{
  hash_table<edge_hasher> edge_htab (EDGE_TABLE_SIZE);
  basic_block bb;
  ...
  if (currently_expanding_to_rtl)
	...
	FOR_EACH_EDGE (e, ei, bb->preds)
	  if (e->insns.r
	      ...
	      && edge_htab.elements () < EDGE_TABLE_SIZE)
	    {
	      hashval_t hash = hash_edge (e);
	      edge *slot = edge_htab.find_slot_with_hash (e, hash, INSERT);
	...
	edge_htab.traverse_noresize <void *, split_edges> (NULL);
