...
rs6000_function_ok_for_sibcall (tree decl, tree exp)
{
  ...
  if (CALL_EXPR_STATIC_CHAIN (exp) || CALL_EXPR_BY_DESCRIPTOR (exp))
    return false;
...
do_stack_clash_protection (void)
{
  ...
  /* If static stack checking is supported and the stack pointer needs to be
...
{
  return (HOST_WIDE_INT_1U << param_stack_clash_protection_probe_interval);
}

static HOST_WIDE_INT
get_stack_clash_protection_guard_size (void)
{
  return (HOST_WIDE_INT_1U << param_stack_clash_protection_guard_size);
}
...
{
  rtx stack_check_symbol = get_stack_check_symbol ();
  ...
  if (stack_check_symbol)
    {
      ...
      rtx cst = GEN_INT (-(first + size));

      /* The add expander doesn't correctly handle r0.  */
      if (satisfies_constraint_I (cst))
	emit_insn (gen_rtx_SET (r0,
				gen_rtx_PLUS (Pmode, stack_pointer_rtx, cst)));
      else
	{
	  emit_move_insn (r0, cst);
	  emit_insn (gen_rtx_SET (r0,
				  gen_rtx_PLUS (Pmode, stack_pointer_rtx, r0)));
	}

      emit_insn (gen_rtx_SET (r12, gen_rtx_HIGH (Pmode, stack_check_symbol)));
      emit_insn (gen_rtx_SET (r12,
			      gen_rtx_MEM (Pmode,
					   gen_rtx_LO_SUM
					   (Pmode, r12,
					    stack_check_symbol))));
      emit_insn (gen_cond_trap (LTU, r0, r12, const0_rtx));
    }

  /* See if we have a constant small number of probes to generate.  If so,
     that's the easy case.  */
  else if (first + size <= 32768)
    {
      HOST_WIDE_INT i;
