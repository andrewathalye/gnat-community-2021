...
#define LINK_OS_VXWORKS_SPEC \
  " %{!mrtp:-r} %{mrtp:-q %{!shared:%{!non-static:-static}}} \
                %{!Xbind-lazy:-z now}"
...
/* It is desired to use the default ppc*-linux emulations here, but the
   MERGE_SECTIONS issue prevents that for now.  */

#undef LINK_OS_LINUX_EMUL32
#define LINK_OS_LINUX_EMUL32 ENDIAN_SELECT(" -m elf32ppcvxworks7r2",    \
                                           " ",                         \
                                           " -m elf32ppcvxworks7r2")

#undef LINK_OS_LINUX_EMUL64
#define LINK_OS_LINUX_EMUL64 ENDIAN_SELECT(" -m elf64ppcvxworks7r2",    \
                                           " ",                         \
                                           " -m elf64ppcvxworks7r2")
