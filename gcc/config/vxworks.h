...
/* For VxWorks static rtps, the system provides libc_internal.a, a superset of
   ...
   that might reference it and libc_internal.  This is not needed when using
   USE_INITFINI_ARRAY since __init and __fini are implemented in the start
   file.
...
#define VXWORKS_BASE_LIBS_RTP "-lc"
#else
...
#define STARTFILE_PREFIX_SPEC                                  \
  "%{shared|non-static:%:getenv(VSB_DIR /usr/lib/common/PIC)}  \
   %:getenv(VSB_DIR /usr/lib/common)"
...
"%{!mrtp:-r}						\
 ...
 %{mrtp:-q %{h*} %{R*} %{!T*: %(link_start)} %(link_os) \
     %{!shared:%{!non-static:-static}}}"

#undef VXWORKS_LINK_SPEC
#define VXWORKS_LINK_SPEC VXWORKS_BASE_LINK_SPEC " " VXWORKS_EXTRA_LINK_SPEC

#undef VXWORKS_LIBGCC_SPEC
/* When either ENABLE_SHARED_LIBGCC or CONFIG_DUAL_EXCEPTIONS is set, the
   static link must be done with libgcc_eh.a or libgcc_sjlj.a.  The choice
   is done in gcc/gcc.c for various combinaison of CONFIG_DUAL_EXCEPTIONS
   and ENABLE_SHARED_LIBGCC.  */
...
#define VXWORKS_LIBGCC_SPEC                                             \
"%{!mrtp: -lgcc}                                                        \
 %{mrtp:%{!static-libgcc:%{shared|non-static:-lgcc_s;: -lgcc}}          \
         %{static-libgcc: -lgcc}}"
#endif /* ENABLE_SHARED_LIBGCC */

/* Setup the crtstuff begin/end we might need for dwarf EH registration and
   for INITFINI_ARRAY setup.  */

#if HAVE_INITFINI_ARRAY_SUPPORT \
  || (!defined(CONFIG_SJLJ_EXCEPTIONS) && DWARF2_UNWIND_INFO)
#define VX_CRTBEGIN_SPEC       \
 "%{!shared:vx_crtbegin.o%s;   \
           :vx_crtbeginS.o%s}"
#define VX_CRTEND_SPEC         \
 "%{!shared:vx_crtend.o%s;     \
           :vx_crtendS.o%s}"
#else
#define VX_CRTBEGIN_SPEC ""
#define VX_CRTEND_SPEC ""
...
/* Help locate headers, of particular use if we run fixincludes for
   this port.  This provides proper rtp/kernel multilib distinction.  */

#undef SYSROOT_HEADERS_SUFFIX_SPEC
#define SYSROOT_HEADERS_SUFFIX_SPEC \
    "%{mrtp:${WIND_USR};:${WIND_BASE}/target}/h"
