...
/* Subroutine of do_jump, dealing with exploded comparisons of the type
   ...
   PROB is probability of jump to if_true_label.  LOC is the location
   to be used for the jump, if known.  */

static void
do_jump_1 (enum tree_code code, tree op0, tree op1,
	   rtx_code_label *if_false_label, rtx_code_label *if_true_label,
	   profile_probability prob, location_t loc)
{
  machine_mode mode;
  ...
  switch (code)
	...
	gcc_assert (GET_MODE_CLASS (TYPE_MODE (inner_type))
	  ...
	  do_jump (op0, if_true_label, if_false_label, prob.invert (), loc);
	...
	gcc_assert (GET_MODE_CLASS (TYPE_MODE (inner_type))
	  ...
	  do_jump_by_parts_equality (int_mode, op0, op1, if_true_label,
				     if_false_label, prob.invert (), loc);
        else
...
/* Given an EQ_EXPR expression EXP for values too wide to be compared
   with one insn, test the comparison and jump to the appropriate label.
   MODE is the mode of the two operands.  LOC is the location to be used
   for the jump, if known.  */
...
{
  do_jump (exp, NULL, label, prob, EXPR_LOCATION (exp));
}
...
jumpif_1 (enum tree_code code, tree op0, tree op1, rtx_code_label *label,
	  profile_probability prob, location_t loc)
{
  do_jump_1 (code, op0, op1, NULL, label, prob, loc);
}
...
{
  do_jump (exp, label, NULL, prob.invert (), EXPR_LOCATION (exp));
}
...
jumpifnot_1 (enum tree_code code, tree op0, tree op1, rtx_code_label *label,
	     profile_probability prob, location_t loc)
{
  do_jump_1 (code, op0, op1, label, NULL, prob.invert (), loc);
}
