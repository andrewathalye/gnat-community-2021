...
/* Return true if the source line of INSN is in the body.  */

static bool
source_line_in_body (rtx_insn *insn)
{
  ...
  if (LOCATION_LOCUS (INSN_LOCATION (insn)) == cfun->function_end_locus)
    return false;
  ...
  switch (GET_CODE (insn))
	...
	case NOTE_INSN_PROLOGUE_END:
	  ...
	  if ((*seen & (SEEN_EMITTED | SEEN_NOTE)) == SEEN_NOTE)
	    {
	      *seen |= SEEN_EMITTED;
	      /* Do not force a source line if there will be one emitted for
		 the body of the function by a subsequent instruction.  */
	      if (!source_line_in_body (NEXT_INSN (insn)))
...
notice_source_line (rtx_insn *insn, bool *is_stmt)
{
  ...
  else if (NOTE_P (insn) && NOTE_KIND (insn) == NOTE_INSN_CODE_LOCATION)
    {
      location_t loc = NOTE_CODE_LOCATION (insn);
      expanded_location xloc = expand_location (loc);
      if (xloc.line == 0)
	{
	  gcc_checking_assert (LOCATION_LOCUS (loc) == UNKNOWN_LOCATION
