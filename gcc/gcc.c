...
#ifdef CONFIG_DUAL_EXCEPTIONS
  {
    const char *p = libgcc_spec;
    int in_sep = 1;

    /* Transform the extant libgcc_spec into one that uses the SJLJ libgcc
       when given the proper command line argument.  */
    while (*p)
      {
#if defined(ENABLE_SHARED_LIBGCC)
	if (in_sep && *p == '-' && strncmp (p, "-lgcc_eh", 8) == 0)
	  {
	    const char *buf = "%{fsjlj:-lgcc_sjlj; :-lgcc_eh}";
	    obstack_grow (&obstack, buf, strlen (buf));
	    p += 8;
	    in_sep = 0;
	  }
	else if (in_sep && *p == 'l' && strncmp (p, "libgcc_eh.a%s", 13) == 0)
	  {
	    const char *buf = "%{fsjlj:libgcc_sjlj.a%s; :libgcc_eh.a%s}";
	    obstack_grow (&obstack, buf, strlen (buf));
	    p += 13;
	    in_sep = 0;
	  }
	...
	if (in_sep && *p == '-' && strncmp (p, "-lgcc", 5) == 0)
	  {
	    const char *buf = "%{fsjlj:-lgcc -lgcc_sjlj; :-lgcc -lgcc_eh}";
	    obstack_grow (&obstack, buf, strlen (buf));
	    p += 5;
	    in_sep = 0;
	  }
	  ...
	  {
	    const char *buf
	      = "%{fsjlj:libgcc.a%s libgcc_sjlj.a%s; :libgcc.a%s libgcc_eh.a%s}";
	    obstack_grow (&obstack, buf, strlen (buf));
	    p += 10;
	    in_sep = 0;
	  }
