...
/* Try to promote the store of SRC, which must be a STRING_CST, to DEST and
   return true on success.  FIRST_STORE is the first original store of the
   group of stores for DEST and VDEF is the virtual operand of the last.  */

static bool
promote_dest_to_static (tree dest, tree src, gimple *first_store, tree vdef)
{
  tree addr = TREE_OPERAND (dest, 0);
  if (TREE_CODE (addr) != SSA_NAME)
    return false;

  /* DEST must be local memory obtained by means of alloca...  */
  gimple *def_stmt = SSA_NAME_DEF_STMT (addr);
  if (!(def_stmt
	&& (gimple_call_builtin_p (def_stmt, BUILT_IN_ALLOCA_WITH_ALIGN)
	    || gimple_call_builtin_p (def_stmt,
				      BUILT_IN_ALLOCA_WITH_ALIGN_AND_MAX))))
    return false;

  /* ...and whose size is equal to the length of the string.  */
  gcc_assert (TREE_CODE (src) == STRING_CST);
  if (!tree_int_cst_equal (gimple_call_arg (def_stmt, 0),
			   TYPE_SIZE_UNIT (TREE_TYPE (src))))
    return false;

  /* The first original store must be the first statement after alloca.  */
  gimple_stmt_iterator gsi = gsi_for_stmt (def_stmt);
  gsi_next_nondebug (&gsi);
  if (gsi_stmt (gsi) != first_store)
    return false;

  /* Create the new variable in static memory.  */
  tree var = build_decl (gimple_location (def_stmt), VAR_DECL,
			 create_tmp_var_name ("A"), TREE_TYPE (dest));
  DECL_ARTIFICIAL (var) = 1;
  DECL_IGNORED_P (var) = 1;
  TREE_STATIC (var) = 1;
  TREE_USED (var) = 1;
  DECL_INITIAL (var) = src;
  varpool_node::add (var);

  /* Replace alloca with a mere address operation.  */
  tree vuse = gimple_vuse (def_stmt);
  gsi = gsi_for_stmt (def_stmt);
  bool ok = update_call_from_tree (&gsi, build_fold_addr_expr (var));
  gcc_assert (ok);

  /* The new statement does not touch memory so the VUSE of the old statement
     must be propagated into the uses of the VDEF of the last store.  */
...
imm_store_chain_info::output_merged_store (merged_store_group *group)
{
  ...
  FOR_EACH_VEC_ELT (split_stores, i, split_store)
      ...
      else
	{
      ...
      /* Try to promote a string concatenation to static memory.  */
      if (group->string_concatenation
	  && promote_dest_to_static (dest, src,
				     split_store->orig_stores[0]->stmt,
				     last_vdef))
	continue;
  ...
  if (!seq)
    {
      gcc_assert (group->string_concatenation);

      if (dump_file)
	fprintf (dump_file,
		 "Static store to replace sequence of %u stores\n",
		 orig_num_stmts);
