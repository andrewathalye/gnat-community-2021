...
/* Return the location to be used for T during gimplification.
   ...
   This is a kludge to accommodate the -fdump-scos switch.  It expects
...
static inline location_t
adjusted_rexpr_location (tree t)
{
  if (TREE_CODE (t) != STATEMENT_LIST)
    return adjusted_expr_location (t);
  ...
  return adjusted_rexpr_location (tsi_stmt (i));
}
  ...
  if (else_se)
      ...
      if (jump_over_else)
	{
	  ...
	  if (rexpr_has_location (last))
	    SET_EXPR_LOCATION (t, adjusted_rexpr_location (last));
	  append_to_statement_list (t, &expr);
	}
      if (emit_false)
	{
  ...
  if (!VOID_TYPE_P (type))
      ...
      else
	{
	 ...
	 assignment if the value is void; in C++ it can be if it's a throw.  */
      if (!VOID_TYPE_P (TREE_TYPE (then_)))
	TREE_OPERAND (expr, 1) = build2_loc (EXPR_LOCATION (then_),
					     INIT_EXPR, type, tmp, then_);

      /* Similarly, build the new else clause, `tmp = else_;'.  */
      if (!VOID_TYPE_P (TREE_TYPE (else_)))
	TREE_OPERAND (expr, 2) = build2_loc (EXPR_LOCATION (else_),
					     INIT_EXPR, type, tmp, else_);
