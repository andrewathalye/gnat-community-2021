		   ...
		   bool early = false)
{
  ...
  /* If we are preserving the CFG, don't inline if source location
     information from the callee would be lost in the caller.  */
  else if (flag_preserve_control_flow
	   && (DECL_IGNORED_P (caller->decl) || caller->thunk.thunk_p)
	   && !DECL_IGNORED_P (callee->decl)
