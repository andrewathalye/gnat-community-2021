...
delete_related_insns (rtx uncast_insn)
{
  ...
  while (prev && (prev->deleted () || NOTE_P (prev)))
    {
      /* Remove code location notes that would leak into another block.  */
      rtx next_nonnote;
      if (NOTE_P (prev)
	  && (NOTE_KIND (prev) == NOTE_INSN_CODE_LOCATION
	      || NOTE_KIND (prev) == NOTE_INSN_BEGIN_STMT
	      || NOTE_KIND (prev) == NOTE_INSN_INLINE_ENTRY)
	  && !(NONJUMP_INSN_P (insn) && GET_CODE (PATTERN (insn)) == SEQUENCE)
	  && (next_nonnote = next_nonnote_nondebug_insn_bb (insn))
	  && (BARRIER_P (next_nonnote) || LABEL_P (next_nonnote)))
	remove_insn (prev);
      prev = PREV_INSN (prev);
    }
