...
/* Move invariant INVNO out of the LOOP.  If CLEAR_LOC is true, also clear its
   location.  Return true if this succeeds, false otherwise.  */

static bool
move_invariant_reg (class loop *loop, unsigned invno, bool clear_loc)
{
  struct invariant *inv = invariants[invno];
  ...
  if (inv == repr)
      ...
      if (inv->depends_on)
	{
	  EXECUTE_IF_SET_IN_BITMAP (inv->depends_on, 0, i, bi)
	    {
	      if (!move_invariant_reg (loop, i, clear_loc))
		goto fail;
	    }
	}
	...
	fprintf (dump_file, "Invariant %d moved without introducing a new "
      ...
      if (clear_loc)
	INSN_LOCATION (inv->insn) = UNKNOWN_LOCATION;
    ...
    {
      if (!move_invariant_reg (loop, repr->invno, clear_loc))
	goto fail;
      reg = repr->reg;
...
move_invariants (class loop *loop)
{
  ...
  /* We clear the location of the moved invariants unless we cannot get it from
     the preheader to avoid having them inherit an unrelated location.  */
  FOR_EACH_VEC_ELT (invariants, i, inv)
    move_invariant_reg (loop, i, loc_from_preheader);
