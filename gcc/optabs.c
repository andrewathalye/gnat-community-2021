...
expand_float (rtx to, rtx from, int unsignedp)
{
  ...
  /* No hardware instruction available; call a library routine.  */
  {
    ...
    rtx libfunc = convert_optab_libfunc (tab, GET_MODE (to), GET_MODE (from));
    if (libfunc)
      {
	start_sequence ();
	rtx value = emit_library_call_value (libfunc, NULL_RTX, LCT_CONST,
					     GET_MODE (to), from,
					     GET_MODE (from));
...
expand_fix (rtx to, rtx from, int unsignedp)
{
  ...
  if (is_narrower_int_mode (GET_MODE (to), SImode))
  ...
  else
    {
      ...
      rtx libfunc
	= convert_optab_libfunc (tab, GET_MODE (to), GET_MODE (from));
      if (libfunc)
	{
	  start_sequence ();
	  rtx value = emit_library_call_value (libfunc, NULL_RTX, LCT_CONST,
	  ...
	  if (unsignedp)
	    {
		 ...
		 a double to hold a uint32_t without rounding.  */
	      ...
	      if (optab_handler (fma_optab, DFmode) == CODE_FOR_nothing)
		{
		  y = expand_binop (DFmode, smul_optab, y, MAXp1, NULL_RTX, 0,
				    OPTAB_LIB_WIDEN);
		  y = expand_binop (DFmode, add_optab, y, from, NULL_RTX, 0,
				    OPTAB_LIB_WIDEN);
		}
	      else
