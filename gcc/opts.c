...
static const struct default_options default_options_table[] =
  {
    ...
    { OPT_LEVELS_1_PLUS, OPT_fallocation_dce, NULL, 1 },
		...
		location_t loc)
{
  ...
  if (opts->x_flag_preserve_control_flow)
	...
	{
	  opts->x_flag_branch_on_count_reg = 0;
	  opts->x_flag_if_conversion = 0;
	  opts->x_flag_if_conversion2 = 0;
	  opts->x_flag_reorder_blocks = 0;
	  opts->x_flag_shrink_wrap = 0;
	  opts->x_flag_ssa_phiopt = 0;
	  opts->x_flag_tree_cselim = 0;
	  opts->x_flag_tree_ch = 0;
	  opts->x_flag_tree_dom = 0;

	  /* We also want to preserve calls to empty functions.  */
	  opts->x_flag_ipa_pure_const = 0;
	}
    }

  /* Disable variable-location-view with -fdump-scos, as it can confuse
     the multi-sloc insn machinery used to handle inlining.  */
  if (opts->x_flag_dump_scos == 1)
    {
      if (opts->x_debug_variable_location_views == 1)
	inform (input_location,
		"-gvariable-location-views is not supported with -fdump-scos");

      opts->x_debug_variable_location_views = 0;
    }
}
		      ...
		      void (*target_option_override_hook) (void))
{
  ...
  switch (code)
    ...
    case OPT_fsjlj:
#ifndef CONFIG_DUAL_EXCEPTIONS
      error ("-fsjlj is available only with --enable-dual-exceptions");
#endif
      break;
