...
bool
preserve_location_for_nop (rtx_insn *insn)
{
  ...
  if (insn == BB_END (bb))
    {
      /* If the block has more than one successor, this means that the insn
	 was added very late and the CFG not updated, so just remove it.  */
      if (!single_succ_p (bb))
	return false;

      /* Preserve the location of edges.  */
      if (INSN_LOCATION (insn) == single_succ_edge (bb)->goto_locus)
	return true;
    }

  /* Preserve the location of final debug insns.  */
  if (insn == BB_END (bb)
      || (JUMP_P (BB_END (bb))
	  && INSN_LOCATION (BB_END (bb)) != INSN_LOCATION (insn)
	  && insn == prev_nonnote_insn (BB_END (bb))))
    {
      rtx_insn *prev = prev_nonnote_insn (insn);
      if (prev
	  && DEBUG_INSN_P (prev)
	  && BLOCK_FOR_INSN (prev) == bb
	  && INSN_LOCATION (prev) == INSN_LOCATION (insn))
	return true;
    }

  /* Preserve the location of SSA fixups generated during expansion.  */
  if (JUMP_P (BB_END (bb))
      && !INSN_HAS_LOCATION (BB_END (bb))
      && insn == prev_nonnote_nondebug_insn_bb (BB_END (bb))
      && prev_nonnote_nondebug_insn_bb (insn) == NULL_RTX)
    return true;

  /* Preserve the location of user returns.  */
  rtx dest = SET_DEST (PATTERN (insn));
  if (REG_P (dest)
      && REG_EXPR (dest) == DECL_RESULT (current_function_decl)
      && known_eq (REG_OFFSET (dest), 0))
    return true;
