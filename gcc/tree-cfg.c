...
make_edges_bb (basic_block bb, struct omp_region **pcur_region, int *pomp_index)
{
  ...
  switch (gimple_code (last))
	...
	if (useful_location_p (cfun, gimple_location (last)))
	  e->goto_locus = gimple_location (last);
	fallthru = false;
      }
      break;
  ...
  if (simple_goto_p (goto_t))
    {
      ...
      if (useful_location_p (cfun, gimple_location (goto_t)))
...
gimple_can_merge_blocks_p (basic_block a, basic_block b)
{
  ...
  if (stmt_a)
    if (glabel *label_stmt = dyn_cast <glabel *> (stmt_a))
  ...
  for (gimple_stmt_iterator gsi = gsi_start_bb (b); !gsi_end_p (gsi);
       gsi_next (&gsi))
    {
      tree lab;
      stmt_b = gsi_stmt (gsi);
      glabel *label_stmt = dyn_cast <glabel *> (stmt_b);
      if (!label_stmt)
	break;
  ...
  if (!optimize_cfg && useful_location_p (cfun, locus))
    {
	...
	return false;
	 ...
	 out later, so pretend that the RETURN holds a unique locus.  */
	  ...
	  && LOCATION_LOCUS (gimple_location (stmt_b))
