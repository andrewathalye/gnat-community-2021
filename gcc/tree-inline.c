  ...
  if (TREE_CODE (*tp) == SSA_NAME)
    {
      ...
      /* Perform constant propagation, even at -O0 if possible.
	 See setup_one_parameter for the rationale.  */
      gimple *def_stmt;
      if (!is_lhs
	  ...
	  && gimple_assign_copy_p (def_stmt)
	  && is_gimple_min_invariant (gimple_assign_rhs1 (def_stmt)))
	*tp = gimple_assign_rhs1 (def_stmt);
      else
	*tp = t;
  ...
  FOR_EACH_EDGE (old_edge, ei, bb->succs)
	...
	if (old_edge->dest->index == EXIT_BLOCK
	    ...
	    && old_edge->dest->aux != EXIT_BLOCK_PTR_FOR_FN (cfun))
	  {
	    ...
	    /* If we are preserving the CFG, preserve the location of user
	       returns of the inlined function.  */
	    if (flag_preserve_control_flow
		&& (stmt = last_stmt (old_edge->src))
		&& gimple_code (stmt) == GIMPLE_RETURN
		&& useful_location_p (id->src_cfun, gimple_location (stmt)))
	      locus = LOCATION_LOCUS (gimple_location (stmt));
	  }
		    ...
		    bitmap to_purge)
{
  ...
  /* If we are preserving the CFG, preserve the location of the call.  */
  if (flag_preserve_control_flow && gimple_has_location (stmt))
    single_succ_edge (bb)->goto_locus = gimple_location (stmt);
