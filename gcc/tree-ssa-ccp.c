...
/* Count the number and the total length of string stores based on ADDR.  */

static struct store_count
count_string_stores (tree addr)
{
  struct store_count sc = { 0, 0 };
  struct imm_use_iterator iter;
  gimple *stmt;
  ...
  FOR_EACH_IMM_USE_STMT (stmt, iter, addr)
	...
	continue;
      ...
      /* This is a store directly based on ADDR.  */
      if (gimple_store_p (stmt) && TREE_CODE (rhs) == STRING_CST)
	{
	  sc.number += 1;
	  sc.length += tree_to_uhwi (TYPE_SIZE_UNIT (TREE_TYPE (rhs)));
	}

      /* Look for stores based on ADDR after offsetting.  */
      else if (TREE_CODE (gimple_assign_lhs (stmt)) == SSA_NAME
	       && TREE_CODE (rhs) == ADDR_EXPR)
	{
	  const struct store_count addend
	    = count_string_stores (gimple_assign_lhs (stmt));
	  sc.number += addend.number;
	  sc.length += addend.length;
	}
    }
...
fold_builtin_alloca_with_align (gimple *stmt)
{
  ...
  /* Do not fold large string concatenations that could be done statically.  */
  if (cfun->after_inlining)
    {
      const struct store_count sc = count_string_stores (lhs);
      if (sc.number > 1 && sc.length > MOVE_MAX)
	return NULL_TREE;
    }
