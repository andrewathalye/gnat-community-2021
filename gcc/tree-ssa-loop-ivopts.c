...
find_givs_in_stmt_scev (struct ivopts_data *data, gimple *stmt, affine_iv *iv)
{
  ...
  /* If STMT can throw internally, then do not consider it as defining a GIV.
...
force_expr_to_var_cost (tree expr, bool speed)
{
  ...
  /* Bound the cost by a multiple of target_spill_cost.  The parts of
     complicated computations often are either loop invariant or at
     least can be shared between iv uses, so letting this grow without
     limits would not give reasonable results for multi-dim arrays.  */
  if (cost.cost > 7 * (int) target_spill_cost [speed])
    cost.cost = 7 * target_spill_cost [speed];
				 ...
				 struct iv_group *group, struct iv_cand *cand)
{
  ...
  if (!sum_cost.infinite_cost_p () && cand->ainc_use == use)
    {
      /* If we are preserving the control flow, do not use autoincrement since
	 this may change the order between the use and update of the IV.  */
      if (can_autoinc && !flag_preserve_control_flow)
  ...
  if (aff.n > 1)
    {
      ...
      for (i = 0; i < aff.n; i++)
	{
	  tree elt = aff.elts[i].val;
	  gimple *def;

	  if (elt != var
	      && TREE_CODE (elt) == SSA_NAME
	      && (def = SSA_NAME_DEF_STMT (elt))
	      && is_gimple_assign (def)
	      && gimple_assign_lhs (def) == elt
	      && gimple_assign_rhs_code (def) == PLUS_EXPR
	      && TREE_CODE (gimple_assign_rhs2 (def)) == INTEGER_CST)
	    {
	      aff_tree current, cst;

	      tree_to_aff_combination (gimple_assign_rhs1 (def), aff.type,
				       &current);
	      aff_combination_const (&cst, aff.type,
				     wi::to_widest (gimple_assign_rhs2 (def)));
	      aff_combination_add (&current, &cst);

	      /* Accumulate the new terms in TO_ADD so that we do not modify
		 AFF while traversing it and include the term -coef * ELT to
		 remove it from AFF.  */
	      widest_int scale = aff.elts[i].coef;
	      aff_combination_scale (&current, scale);
	      aff_combination_add (&to_add, &current);

	      aff_combination_zero (&current, aff.type);
	      aff_combination_add_elt (&current, elt, -scale);
	      aff_combination_add (&to_add, &current);
