...
substitute_and_fold_engine::substitute_and_fold (basic_block block)
{
  ...
  while (!walker.stmts_to_remove.is_empty ())
      ...
      else
	{
	  ...
	  /* Defining statements of SSA_NAMEs whose single use is trivially
	     dead are also trivially dead.  Eliminate them now so the next
	     CFG cleanup runs on a reasonably stable IR.  */
	  if (is_gimple_assign (stmt)
	      && gimple_assign_rhs_class (stmt) == GIMPLE_UNARY_RHS)
	    {
		  ...
		  && !gimple_visited_p (def_stmt))
		{
		  walker.stmts_to_remove.safe_push (def_stmt);
