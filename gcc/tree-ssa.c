...
struct walk_info_data
{
  ...
  /* Bitmap of variables that need a non-register.  */
...
get_base_of_component_ref (tree ref)
{
  ...
  /* Strip isomorphic MEM_REFs around the base.  */
  if (TREE_CODE (base) == MEM_REF)
    {
      tree op0 = TREE_OPERAND (base, 0);

      if (TREE_CODE (op0) == ADDR_EXPR
	  && TYPE_CANONICAL (TREE_TYPE (base))
	     == TYPE_CANONICAL (TREE_TYPE (TREE_OPERAND (op0, 0)))
...
static tree
lookup_decl_for_field (tree field, hash_map<tree, tree> *map,
		       bitmap not_reg_needs, bitmap suitable_for_renaming)
{
  tree *slot = &map->get_or_insert (field);
  if (!*slot)
    {
	...
	{
	  SET_DECL_ALIGN (decl, DECL_ALIGN (field));
	  DECL_USER_ALIGN (decl) = 1;
	}
      else
	SET_DECL_ALIGN (decl, TYPE_ALIGN (TREE_TYPE (field)));
      DECL_IGNORED_P (decl) = DECL_IGNORED_P (field);
      TREE_ADDRESSABLE (decl) = !DECL_NONADDRESSABLE_P (field);
      TREE_NO_WARNING (decl) = TREE_NO_WARNING (field);
      TREE_THIS_VOLATILE (decl) = TREE_THIS_VOLATILE (field);
      TREE_USED (decl) = 1;
      gimple_add_tmp_var (decl);
      /* If the variable's type is scalar, then we need to explicitly mark it
	 as either needing a non-register or to be put into SSA form.  */
...
split_nonlocal_frames_op (tree *tp, int *walk_subtrees, void *info)
{
  ...
  while (handled_component_p (*tp))
	...
	&& !bitmap_bit_p (data->addresses_taken, DECL_UID (base)))
      {
	/* We cannot look up TREE_OPERAND (*tp, 1) directly because the ICF
	   pass might have unified two frame objects with different types.  */
	tree frame_type = TREE_TYPE (base);
	tree offset = byte_position (TREE_OPERAND (*tp, 1));
	tree field, next;

	/* Look for a field in the frame that spans the offset.  */
	...
	tree decl
	  = lookup_decl_for_field (field, data->field_map,
				   data->not_reg_needs,
  ...
  if (TREE_CODE (*tp) == MEM_REF
	...
	if (tree_int_cst_lt (offset, byte_position (next)))
      ...
      /* However, there is one problematic case: if the field can be promoted
	 to a register but the MEM_REF's type is not scalar and the sizes are
	 different, then this would give rise to an invalid VIEW_CONVERT_EXPR.
	 This can happen for code manipulating types with variant part in Ada
	 and that is unreachable.  In this case, reuse the first non-scalar
	 field in the frame, there must be one.  */
      if (DECL_NONADDRESSABLE_P (field)
	  && is_gimple_reg_type (TREE_TYPE (field))
	  && !is_gimple_reg_type (type)
	  && TYPE_SIZE (TREE_TYPE (field)) != TYPE_SIZE (type))
	{
	  for (field = TYPE_FIELDS (frame_type);
	       field;
	       field = DECL_CHAIN (field))
	    if (!is_gimple_reg_type (TREE_TYPE (field)))
	      break;
	  gcc_assert (field);
	}
	...
	offset = fold_convert (TREE_TYPE (addr), offset);
      offset = int_const_binop (MINUS_EXPR, offset, byte_position (field));

      if (addrp && integer_zerop (offset))
	*addrp = addr;
      else
	{
	  tree old = *tp;
	  *tp = fold_build2 (MEM_REF, type, addr, offset);
...
/* Fixup decls in BLOCK and subblocks if fields of non-local frame structures
   are involved.  */

static void
fixup_decls (tree block, struct walk_info_data *data)
{
  for (tree decl = BLOCK_VARS (block); decl; decl = DECL_CHAIN (decl))
    if (VAR_P (decl) && DECL_HAS_VALUE_EXPR_P (decl))
      {
	tree val = DECL_VALUE_EXPR (decl);
	bool indirect_p;

	/* See tree-nested.c:fixup_vla_decls for the rationale.  */
	...
	if (!(TREE_CODE (val) == COMPONENT_REF
	      ...
	      && DECL_NONLOCAL_FRAME (TREE_OPERAND (val, 0))
	      && !bitmap_bit_p (data->addresses_taken,
				DECL_UID (TREE_OPERAND (val, 0)))))
	  continue;

	tree *slot = data->field_map->get (TREE_OPERAND (val, 1));
	if (!slot)
	  continue;

	if (indirect_p)
	  val = build1 (INDIRECT_REF, TREE_TYPE (decl), *slot);
	else
	  val = *slot;

	SET_DECL_VALUE_EXPR (decl, val);
      }

  for (tree sub = BLOCK_SUBBLOCKS (block); sub; sub = BLOCK_CHAIN (sub))
    fixup_decls (sub, data);
}

/* Compute TREE_ADDRESSABLE and DECL_GIMPLE_REG_P for local variables and
   return whether their call-clobber states need to be recomputed.  */
  ...
  if (split_nonlocal_frames)
    {
      ...
      data.field_map = new hash_map<tree, tree>;
      data.addresses_taken = addresses_taken;
      data.not_reg_needs = not_reg_needs;
	...
	for (gimple_stmt_iterator gsi = gsi_start_bb (bb); !gsi_end_p (gsi);
      ...
      fixup_decls (DECL_INITIAL (current_function_decl), &data);
      delete data.field_map;
      ret = true;
    }
