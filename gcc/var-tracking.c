...
static inline void
delete_vta_debug_insn (rtx_insn *insn, rtx_insn *last)
{
  if (DEBUG_MARKER_INSN_P (insn))
    {
      reemit_marker_as_note (insn);
  ...
  if (TREE_CODE (decl) == LABEL_DECL
  ...
  else
    {
	 ...
	 were replaced by debug insns.  */
      if (flag_preserve_control_flow
	  && last
	  && TREE_CODE (decl) != DEBUG_EXPR_DECL
	  && GET_CODE (INSN_VAR_LOCATION_LOC (insn)) != CLOBBER
	  && INSN_HAS_LOCATION (insn)
	  && LOCATION_LOCUS (INSN_LOCATION (insn))
	     != LOCATION_LOCUS (INSN_LOCATION (last)))
	{
	  rtx_insn *note = emit_note_before (NOTE_INSN_CODE_LOCATION, insn);
	  NOTE_CODE_LOCATION (note) = INSN_LOCATION (insn);
	}
  ...
  if (use_cfg)
	...
	FOR_BB_INSNS_REVERSE_SAFE (bb, insn, next)
	  if (DEBUG_INSN_P (insn))
	    delete_vta_debug_insn (insn, last);
	...
	next = NEXT_INSN (insn);
	if (DEBUG_INSN_P (insn))
	  delete_vta_debug_insn (insn, NULL);
      }
}
