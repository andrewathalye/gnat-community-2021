...
do_linemarker (cpp_reader *pfile)
{
  ...
  if (CPP_OPTION (pfile, preserve_system_token_columnno))
    {
      location_t loc_before_directive = pfile->last_line_slocs[2];
      linenum_type old_lineno = SOURCE_LINE (map, loc_before_directive);
      linenum_type old_column = SOURCE_COLUMN (map, loc_before_directive);

      /* If the line before this line directive has the same filename and line
	 than the next ones, restore the column count to continue this previous
	 line.  */
      if (old_lineno == new_lineno
	  && !strcmp(new_file, ORDINARY_MAP_FILE_NAME (map)))
	pfile->column_offset = old_column + 1;
    }
}
