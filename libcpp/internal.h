...
#define CPP_INCREMENT_LINE(PFILE, COLS_HINT) do { \
    ...
    PFILE->column_offset = 0; \
    linemap_line_start (PFILE->line_table, line + 1, COLS_HINT); \
    PFILE->last_line_slocs[2] = PFILE->last_line_slocs[1]; \
    PFILE->last_line_slocs[1] = PFILE->last_line_slocs[0]; \
    PFILE->last_line_slocs[0] = 0; \
  } while (0)
...
struct cpp_reader
{
  ...
  /* Initial column number for the current line.  To be added to
     CPP_BUF_COLUMN.  */
  unsigned column_offset;

  /* Rotating buffer for the last slocs of the three last lines (including
     current).  Most recent line is at index 0, oldest one at index 2.  This is
     used to implement the preserve_system_token_columnno option.  */
  location_t last_line_slocs[3];
